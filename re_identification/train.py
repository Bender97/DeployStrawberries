...
class CrossValidationSetup:
    def __init__(self, foldidx, seedidx, mode, datapath, iou_th=None):
        ...
        if self.mode=="kfold_crossvalidation":
            ...
            self.test_from  = Strawberries(f"{self.datapath}/14_21/strawberries_14", os.path.join(self.datapath, "selections_1.json"),  Ts["14"], min_x=27.58, max_x=29.45,    training=False)
            self.test_next  = Strawberries(f"{self.datapath}/14_21/strawberries_21", os.path.join(self.datapath, "selections_2.json"),  Ts["21"], min_x=27.58, max_x=29.45,    training=False)
            ...
            print("|** instance segmentation test set summary")
            self.test_from  = Strawberries(f"{self.datapath}/14_21_inst@{iou_th}/strawberries_14", os.path.join(self.datapath, f"14_21_inst@{iou_th}/selections_1.json"),  Ts["14"], min_x=27.58, max_x=29.45,    training=False)
            self.test_next  = Strawberries(f"{self.datapath}/14_21_inst@{iou_th}/strawberries_21", os.path.join(self.datapath, f"14_21_inst@{iou_th}/selections_2.json"),  Ts["21"], min_x=27.58, max_x=29.45,    training=False)
            self.testloader_from = DataLoader(self.test_from, batch_size=len(self.test_from), shuffle=False,  collate_fn=self.test_from.custom_collation_fn)
            self.testloader_next = DataLoader(self.test_next, batch_size=len(self.test_next), shuffle=False,  collate_fn=self.test_next.custom_collation_fn)
    ...
    def run_test(self):
        ...
        import pickle
        with open("instsegm_gcnconv_2.pickle", "wb") as handle:
